      ...
      {items.map((value, key) => (
        <div key={key} className="mb-5">
          <div className="border flex h-28 justify-between p-10 rounded-2xl text-xl">
            ...
            <div className="text-grey-400 text-sm rounded-full bg-grey-200 w-20 py-1 align-items-center text-center">
              {value.address}
            </div>
            <div className="font-bold">{tr('profile_address_change')}</div>
          </div>
        </div>
      ))}
    </>
